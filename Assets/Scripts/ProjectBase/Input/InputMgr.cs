...
public class InputMgr : BaseManager<InputMgr>
{
    ...
    private void CheckKeyCode(KeyCode key)
    {
        ...
        if (Input.GetKeyDown(key))
            EventCenter.GetInstance().EventTrigger<object>("某键按下", key);
        //事件中心模块 分发按下抬起事件
        if (Input.GetKeyUp(key))
            EventCenter.GetInstance().EventTrigger<object>("某键抬起", key);
    }
    private void MyUpdate()
    {
        if (!isStart || !GamingLifecycleMgr.CanInput)
            return;

        if (MouseStateMgr.GetInstance().IsMouseRequired())
            return;
